#!/usr/bin/env node
// VG Brain SessionEnd hook — POST the raw transcript to /ingest at session close.
// Exits 0 on every path; capture must never block a session from closing.
import { readFileSync, readdirSync, statSync } from "node:fs";
import { request as httpsRequest } from "node:https";
import { request as httpRequest } from "node:http";

function bail(msg) {
  if (msg) process.stderr.write("[vg-brain session-end] " + msg + "\n");
  process.exit(0);
}
